...
#include "../External/Detour/include/DebugDraw.h"
...
bool InputGeom::load(rcContext* ctx, const std::wstring& filepath)
{
	...
	if (extension == L".gset")
		return loadGeomSet(ctx, WStringToStringReturn(filepath));
	if (extension == L".obj")
		return loadMesh(ctx, WStringToStringReturn(filepath));
