...
//		void SetPosition(const Vector3& position) { mRelativePosition = position; mWorldPosition = position; }
	...
	Vector4 GetWorldLTRB() const {
		return Vector4(mWorldPosition.x - mWorldScale.x * 0.5f, mWorldPosition.y + mWorldScale.y * 0.5f
	...
	void SetPhysicalRotation_For_Static(const Vector3& rotation_degrees);
