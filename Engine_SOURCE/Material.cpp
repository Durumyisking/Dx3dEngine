...
namespace dru::graphics
{
	...
	Material::Material(std::wstring _TextureName, std::wstring _ShaderName)
		...
		, mMode(eRenderingMode::Transparent) 
	{
		mTexture[static_cast<UINT>(eTextureSlot::T0)] = GETSINGLE(ResourceMgr)->Find<Texture>(_TextureName);
		mShader = GETSINGLE(ResourceMgr)->Find<Shader>(_ShaderName);
	...
	Material::Material(std::wstring _TextureName, eTextureSlot _Slot, std::wstring _ShaderName)
		...
		, mMode(eRenderingMode::Transparent)
	{
		mTexture[static_cast<UINT>(_Slot)] = GETSINGLE(ResourceMgr)->Find<Texture>(_TextureName);
		mShader = GETSINGLE(ResourceMgr)->Find<Shader>(_ShaderName);
		mConstantBuffer.xyzw1 = Vector4{ 1.f, 1.f, 1.f, 1.f };
	...
	{
		Shader* shader = GETSINGLE(ResourceMgr)->Find<Shader>(_Key);
		mShader = shader;
	}
}
