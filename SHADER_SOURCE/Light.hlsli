...
float3 CalculateLightPBR_Direct(float3 viewPos, float4 albedo, float3 viewNormal, float metallic, float roughness)
{
    ...
    float3 prefilteredColor = prefilteredMap.SampleLevel(skyBoxSampler, -N, roughness * MAX_REFLECTION_LOD).rgb;
...
float3 CalculateLightPBR_Diffuse(float3 viewPos, float4 albedo, float3 viewNormal, float metallic, float roughness, float3 n)
{
    // PBR     
